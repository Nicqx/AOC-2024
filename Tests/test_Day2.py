...
from Days.Day2 import Day2, check_distance, check_if_monotone
...
class TestDay2(TestCase):
    ...
    def test_check_decrease(self):
        arr_true = [7, 6, 4, 2, 1]
        expected_value = True
        actual_value = check_if_monotone(arr_true, False)
        self.assertEqual(expected_value, actual_value)

    def test_check_increase(self):
        arr_true = [1, 2, 4, 6, 7]
        expected_value = True
        actual_value = check_if_monotone(arr_true, True)
        self.assertEqual(expected_value, actual_value)

    def test_check_decrease2(self):
        arr_false = [7, 6, 4, 5, 1]
        expected_value = False
        actual_value = check_if_monotone(arr_false, False)
        self.assertEqual(expected_value, actual_value)
    ...
    def test_check_increase2(self):
        arr_false = [1, 5, 4, 6, 7]
        expected_value = False
        actual_value = check_if_monotone(arr_false, True)
        self.assertEqual(expected_value, actual_value)
